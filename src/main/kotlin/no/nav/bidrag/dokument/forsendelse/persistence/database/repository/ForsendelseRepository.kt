...
interface ForsendelseRepository : CrudRepository<Forsendelse, Long> {
    ...
    @Query("select f from forsendelse f where f.status = 'DISTRIBUERT' and f.distribusjonKanal = 'NAV_NO' and f.distribuertTidspunkt <= :olderThan and f.distribuertTidspunkt >= :newerThan order by f.distribuertTidspunkt asc")
    fun hentDistribuerteForsendelseTilNAVNO(pageable: Pageable, olderThan: LocalDateTime, newerThan: LocalDateTime): List<Forsendelse>
