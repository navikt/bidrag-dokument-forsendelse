...
interface DokumentRepository : CrudRepository<Dokument, Long> {
    ...
    @Query("select d from dokument d where d.forsendelse.status = 'UNDER_PRODUKSJON' and d.slettetTidspunkt is null and d.dokumentStatus = 'UNDER_REDIGERING' and d.arkivsystem != 'FORSENDELSE' and d.opprettetTidspunkt <= :olderThan order by d.opprettetTidspunkt desc")
