...
interface DokumentRepository : CrudRepository<Dokument, Long> {
    ...
    @Query("select d from dokument d where d.forsendelse.status = 'UNDER_PRODUKSJON' and d.slettetTidspunkt is null and d.dokumentStatus = 'UNDER_REDIGERING' and d.opprettetTidspunkt >= :newerThan and d.opprettetTidspunkt <= :olderThan order by d.opprettetTidspunkt asc")
    fun hentDokumentIkkeFerdigstiltFørOgEterDato(pageable: Pageable, newerThan: LocalDateTime, olderThan: LocalDateTime): List<Dokument>
}
