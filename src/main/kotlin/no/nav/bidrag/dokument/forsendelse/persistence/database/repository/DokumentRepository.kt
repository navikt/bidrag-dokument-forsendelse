...
interface DokumentRepository : CrudRepository<Dokument, Long> {
    ...
    @Query("select d from dokument d where d.forsendelse.status = 'UNDER_PRODUKSJON' and d.slettetTidspunkt is null and d.dokumentStatus = 'UNDER_REDIGERING' and d.arkivsystem != 'FORSENDELSE' and d.opprettetTidspunkt <= :beforeDate and d.opprettetTidspunkt > :afterDate order by d.opprettetTidspunkt desc")
    fun hentDokumentIkkeFerdigstiltFørDato(pageable: Pageable, beforeDate: LocalDateTime, afterDate: LocalDateTime): List<Dokument>
}
